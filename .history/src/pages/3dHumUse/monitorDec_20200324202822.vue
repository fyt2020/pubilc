      ...
      <div class="right">
        <div class="r__title">
          ...
          <div class="r__titleR">
            ...
            <el-button type="primary" size="mini" round class="screen" @click="screen" v-if="percent == 100">筛选</el-button>
          </div>
        </div>
           ...
           <ul :class="['right__conBox']" v-if="isScreen">
            <template v-for="(item,index) in videoArr ">
              <li class="right__item" :key="index" @click="goTo(item)">
                <div class="right__left">
                  <img
                    :src="`data:image/png;base64,${item.angleImg}`"
                    alt
                    class="item__img"
                  />
                  <!-- <img :src="`${imgUrl}/${item.list[0].angleImg}`" alt class="item__img" /> -->
                ...
                <div class="right__mid">
                  ...
                  <p
                    class="isLibrary__box"
                    v-if="1"
                  >相似度:{{((item.possibility* 100).toFixed(2))}}%</p>
                </div>
          ...
          if (data.code == 0) {
            if (data) {
              if (data.result.length > 0) {
                ...
                VM.$Message.success(`根据人脸库筛选成功,共有${data.result.list.length}个结果！`);
              } else {
                VM.screenData.key = !this.screenData.key;
                VM.$refs.screen.reset();
