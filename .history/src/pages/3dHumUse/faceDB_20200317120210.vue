    ...
    <div class="faceDB__content">
      ...
      <div class="shoot-inp">
        ...
        <div class="shoot_right">
          ...
          <button class="addDec" @click="goToAddMo" v-if="power.get(5013)">导入人脸</button>
        </div>
      </div>
      <div class="dec-list">
        <el-table :data="tableData" stripe style="width: 100%" highlight-current-row>
          ...
          <el-table-column prop="username" label="姓名" width="150" height="40" align="center" :show-overflow-tooltip='true'></el-table-column>
          <el-table-column prop="sex" label="性别" width="200" height="40" align="center" :show-overflow-tooltip='true'></el-table-column>
          <el-table-column prop="belong" label="所属" width="200" height="40" align="center" :show-overflow-tooltip='true'></el-table-column>
          <el-table-column prop="tag" label="标签" width="200" height="40" align="center" :show-overflow-tooltip='true'>
              <template slot-scope="scope">
              <span>{{scope.row.tag.join(',')}}</span>
            </template>
          </el-table-column>
          <el-table-column prop="source" label="数据来源" width="200" height="40" align="center" :show-overflow-tooltip='true'>
            <template slot-scope="scope">
              <span>{{scope.row.source == 0 ? '模型导入': '图片导入'}}</span>
            </template>
          </el-table-column>

          <el-table-column prop="status" label="状态" width="150" height="40" align="center" :show-overflow-tooltip='true'>
              <template slot-scope="scope">
              <span>{{ scope.row.status == 0 ? '未生成': (  scope.row.status == 1 ? '生成中' : (  scope.row.status == 2 ? '成功' : '失败'))  }}</span>
            </template>
          </el-table-column>
         
          <el-table-column prop="time" label="入库时间" width="160" height="40" align="center" :show-overflow-tooltip='true'>
            <!-- <template slot-scope="scope">
          ...
          <el-table-column fixed="right" label="操作" width="220" height="40" align="center">
            <template slot-scope="scope">
              <el-button @click="handleClick(scope.row)" type="text" size="small" class="tableBtn" v-if="power.get(5016)">删除</el-button>
  ...
  methods: {
            ...
            if (res.data.code === 0) {
              ...
              if (data.resourceList && data.resourceList.length > 0) {
                data.resourceList.forEach(item => {
                  ...
                  if (key) {
                    resourceList.set(item.id, key);
                  } else {
                    resourceList.set(item.id, false);
                  }
                });
              }
              VM.set_power({ val: resourceList });
              console.log(VM.power.get(5001))
            }
          })
    ...
    rearch() {
      ...
      this.$http.api
        .get(
          window.BASEURL.testUrl1 +
            `parent-police/api/v1/face?&pageNum=${1}&pageSize=${VM.pagesize}&fieldFilter=${VM.name}&libId=${VM.id}&flag=1&sex=${VM.sex}`
        )
        .then(function(res) {
          if (res.data.code === 0) {
            let data = res.data.result.list;
    ...
    goTo(data) {
      ...
      localStorage.setItem('personId', JSON.stringify({id:data.id,name:this.crumsData.thr,fid:this.id}));
      this.changNow({ val: 4 });
    },
    handleClick(data){
    ...
    deleteItem(data) {
      ...
      this.$http.api
        .put(
          window.BASEURL.testUrl1 +
            `parent-police/api/v1/face/${data}`,parmas
        )
        .then(function(res) {
          if (res.data.code === 0) {
