  ...
  <div class="faceDBdetail__wrapper">
    ...
    <div class="faceDBdetail__content">
      <div class="faceDBdetail__title">
        人脸照片
        <span class="sub">共<span class="sub__num">{{ uploadList.length || modelList }}</span>张</span>
      </div>
        ...
        .then(function(res) {
          ...
          if (res.status === 200) {
              if(source == 0){
                 ...
                 if(VM.nameArr1.length > 24){ //如果图片多于24张
          ...
          if (res.status === 200) {
            ...
            Zip.loadAsync(file).then(function(zip) {
              zip.forEach(function(relativePath, zipEntry) {
                if (zipEntry.name == `${zipEntry.name.split("/")[0]}/`) return;
