            ...
            <ul class="sreach__box">
              <template v-for="(item,index) in currentObj">
                <li class="sreach__item" :key="item.id" v-show="item.key">
                  <el-select v-model="item.value1" placeholder="请选择" class="search__elec" :disabled="item.value1 != ''">
                    <el-option
...
export default {
        ...
        shortcuts: [
          {
            ...
            value() {
              ...
              const a =  start.getTime() - new Date(new Date(new Date().toLocaleDateString()).getTime()).getTime();
              console.log(a)
          ...
          },
          {
            ...
            value() {
              ...
              const start = new Date();
                            const a =  start.getTime() - new Date(new Date(new Date().toLocaleDateString()).getTime()).getTime();
              start.setTime( start.getTime() - 3600 * 1000 * 24 * 365  - a );
              return [start, end];
            }
          }
        ]
    ...
    choseArr() {
      ...
      newArr1.forEach((item, index) => {
        if (VM.statusObj[item.value1]) {
          ...
          if (item.value1 === "一正两侧" || item.value1 === "建模状态" || item.value1 === "数据来源" ) {
  ...
  methods: {
    init() {
      ...
      this.dateVal =  [new Date( new Date(new Date().toLocaleDateString()).getTime() ),new Date(new Date(new Date().toLocaleDateString()).getTime() +24 * 60 * 60 * 1000 - 1 )];
      this.showTimePanel1();
      this.getOrg();
