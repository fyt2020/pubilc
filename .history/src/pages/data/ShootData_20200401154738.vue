...
export default {
  ...
  methods: {
     ...
     */
    sub(data, index) {
      console.log('this.choseIndex',this.choseIndex);
      ...
      this.condition[this.choseIndex].isChoose = false;
    ...
    exportExcel() {
      ...
      this.tableData.forEach((item, index) => {
        ...
        let arr1 = [index + 1, item.areaName, item.orgName, item.cityName, item.sum];
        item.list.forEach(v => {
          let k = null;
          for (let i in v) {
            k = v[i];
      ...
      let titleArr = ["序号","所属区域","采集单位","所在城市","合计"].concat(dayArr);
