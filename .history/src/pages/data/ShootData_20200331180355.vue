              ...
              <template v-for="(item,index) in currentObj">
                <li class="sreach__item" :key="item.id" v-show="item.key">
                  ...
                  <div class="inp__BOX">
                    <el-select
                      class="search__elec2"
                      v-if="item.value1 === '标签' || item.value1 === '性别' || item.value1 === '数据来源'"
                      v-model="item.value3"
                    ...
                    <input type="text" class="inp" v-model="item.value3" v-if="item.value1 === '采集数量'" />
                  </div>
          ...
          <template v-if="sumKey">
            ...
            >
              <template slot-scope="scope" v-if="sumKey">
                <span>{{scope.row.days[index][item]}}</span>
              </template>
            </el-table-column>
          </template>
        </el-table>
...
export default {
    ...
    return {
      ...
      condition: [
        ...
        },
        {
          ...
          isChoose: false,
          id: 105,
          son: [{ value: "建模失败" }, { value: "建模成功" }, { value: "建模中" },{value: "排队中" }],
          grdSon:[]
        }
      ] //筛选条件,
    };
  ...
  methods: {
    init() {
      ...
      this.getOrg();
      console.log(encodeURIComponent(JSON.stringify([{'screenFiled':'sex','operation':0,'value':'男'}])))
    },
    ...
    showTimePanel1() {
      ...
      // console.log(new Date(this.dateVal[0]).getTime());
      // console.log(new Date(this.dateVal[1]).getTime());
    },
    /**
     * @Author fyt
     * @Description 选择第一个条件
    ...
    found() {
      ...
      let choseArr = this.currentObj.filter((item)=>{
      ...
      this.$http.api
        .get(
          window.BASEURL.testUrl1 +
            `parent-police/api/v1/taskStatistics?startTime=${VM.startTime}&endTime=${VM.endTime}&orgId=${VM.orgId}&areaIds=${encodeURIComponent(VM.areaIndex)}&type=${VM.type}&screen=${VM.screen.length == 0 ?'%5B%5D':encodeURIComponent(VM.screen)}`
        )
        .then(function(res) {
          if (res.data.code === 0) {
            let data1 = res.data.result;
            let dayType = VM.type == 0 ? 'days' : (VM.type == 1 ? 'weeks':(VM.type == 2 ? 'months': (VM.type == 3 ? 'years': '')))
            // console.log(data1)
            let len = data1.length;
            if (len > 0) {
              VM.day = data1[0].days.map(item => {
                let k = null;
                for (let i in item) {
                  k = i;
                }
    ...
    exportExcel() {
      ...
      let dayArr = this.tableData[0].day.map(item => {
        let k = null;
        for (let i in item) {
          k = i;
        }
      ...
      this.tableData.forEach((item, index) => {
        let arr1 = [index + 1, item.orgName, item.sum];
        item.day.forEach(v => {
          let k = null;
          for (let i in v) {
            k = v[i];
      ...
      let titleArr = ["序号", "采集单位", "合计"].concat(dayArr);
