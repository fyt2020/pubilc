    ...
    <div class="shootData__con">
      ...
      <div class="sreach__condition">
        ...
        <div class="condition__item">
          ...
          <div class="inp_box">
            <el-select v-model="deviceData.area" placeholder="请选择"  class="search__elec">
              <el-option
            ...
            <ul class="sreach__box">
              <template v-for="(item,index) in currentObj">
                <li class="sreach__item" :key="item.id" v-show="item.key">
                  <el-select v-model="item.value1" placeholder="请选择"  class="search__elec2">
                    <el-option
                  ...
                  <el-select v-model="item.value2" placeholder="请选择"  class="search__elec2">
                    <el-option
  ...
  computed: {
    choseArr() {
      ...
      newArr1.forEach((item, index) => {
        ...
        if (item.value1 === "一正两侧" || item.value1 === "建模状态") {
          obj["value"] = "";
        ...
        .get(
          window.BASEURL.testUrl1 +
            `parent-police/api/v1/taskStatistics?startTime=${
              ...
              VM.screen.length == 0 ? "%5B%5D" : encodeURIComponent(JSON.stringify(VM.screen))
            }`
        )
        .then(function(res) {
          if (res.data.code === 0) {
            ...
            if (len > 0) {
              VM.day = data1[0][dayType].map(item => {
                let k = null;
                for (let i in item) {
                  k = i;
                }
