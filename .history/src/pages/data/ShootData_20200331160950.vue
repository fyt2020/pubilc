...
export default {
  ...
  methods: {
    init() {
      ...
      this.getOrg();
      console.log(encodeURIComponent([{name:'dasd'}]))
    },
      ...
      this.$http.api
        .get(
          window.BASEURL.testUrl1 +
            `parent-police/api/v1/taskStatistics?startTime=${VM.startTime}&endTime=${VM.endTime}&orgId=${VM.orgId}&areaIds=${encodeURIComponent(VM.areaIds)}&type=${VM.type}&screen=${VM.screen.length == 0 ?'%5B%5D':encodeURIComponent(VM.screen)}`
        )
        .then(function(res) {
          if (res.data.code === 0) {
     ...
     */
    selectArea(data) {
      this.areaIds = `${data.area_index},${data.area_code}`;
