...
export default {
  ...
  methods: {
    init() {
      ...
      // console.log(encodeURIComponent('20009,20009,23212'))
    },
      ...
      this.$http.api
        .get(
          window.BASEURL.testUrl1 +
            `parent-police/api/v1/taskStatistics?startTime=${VM.startTime}&endTime=${VM.endTime}&areaIds=${encodeURIComponent(VM.areaIds)}&type=${VM.type}&screen=${encodeURIComponent(VM.screen)}`
        )
        .then(function(res) {
          if (res.data.code === 0) {
