  ...
  <div class="addDec-wrapper">
    <div class="addDec-header">
      ...
      <div class="addDec-content">
          ...
          <div class="one">
            <div class="inp-con">
              <label for="one">
                ...
                <input type="text" id="one" class="inp" v-model="network" placeholder="如:192.168.1" autocomplete="off"/>
              </label>
            </div>
            <div class="inp-con">
              <label for="one">
                <span class="cen">主机地址范围</span>
                <input type="text" id="one" class="inp1" v-model="start" autocomplete="off"/>
                <Icon type="ios-remove" />
                <input type="text" id="one" class="inp1" v-model="end" autocomplete="off"/>
              </label>
            </div>
            ...
            <div class="inp-con">
              <label for="one">
                ...
                <input type="text" id="one" class="inp" v-model="customIp" placeholder="如:20,30" autocomplete="off" />
              </label>
            </div>
            <div class="inp-con">
                <button  :class="['link-btn',{'deg':isSend}]"  @click="addDec">
        ...
        <!-- <div class="dec-list" v-if="showTable2">
          <el-table :data="tableData" stripe style="width: 100%" highlight-current-row>
            <el-table-column prop="index" label="序号" width="100" height="40"></el-table-column>
            <el-table-column prop="version" label="摄像头编号" width="400" height="40"></el-table-column>
            <el-table-column prop="deviceNo" label="IP地址" width="300" height="40"></el-table-column>
            <el-table-column prop="orgName" label="描述" height="40"></el-table-column>
          </el-table>
        </div> -->
...
export default {
  ...
  data() {
    return {
      ...
      deviceIp: "192.168.1.71:8000", //设备ip
      network: "", //网段
      ipScope: "", //范围
      customIp: "", //自定义
      strUrl: "192.168.20.30:8080",//视频流镜头IP地址
      start: "",
      end: "",
      labelArr: "禁用",
      ...
      this.$http.api
        ...
        .then(function(res) {
          if (res.data.code === 0) {
            ...
            // console.log(res.data.result[0].cameras)
          ...
          if (res.data.code == 0) {
            ...
            if(VM.badDevArr.length > 0){
              ...
              if(VM.badDevArr.length == 0){
               ...
               VM.decStatus = `${res.data.msg},拍照服务未启动`
               VM.loading2 = false;
          }
        })
        .catch(function(error) {
